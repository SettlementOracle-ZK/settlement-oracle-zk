...
| # | Tarefa | Camada |
|---|--------|--------|
| 2.1 | **P** Instruction `evaluate_trigger` (CPI Pyth on-chain ou flag off-chain) | `programs/` |
| 2.2 | **P** Testes on-chain: trigger happy path + oracle stale → sem payout | `programs/` |
| 2.3 | **S** Revisar PR payout do Klisman (`execute_payout`, `pause`) | `programs/` |
| 2.4 | **S** Scaffold `api/` (rust + PostgreSQL) | `api/` |
| 2.5 | **S** Endpoint `GET /health` + conexão DB | `api/` |

### Klisman · Primary `execute_payout` (on-chain) + API | Secondary trigger

| # | Tarefa | Camada |
|---|--------|--------|
| 2.6 | **P** Instruction `execute_payout` + circuit breaker `pause`/`unpause` | `programs/` |
| 2.7 | **P** Testes on-chain: payout happy path + pause bloqueia payout | `programs/` |
| 2.8 | **P** Schema DB: `policies`, `settlements` | `api/` |
| 2.9 | **P** `GET /policies/:id` + integração RPC Solana (ler escrow) | `api/` |
| 2.10 | **S** Revisar PR `evaluate_trigger` do Rodrigo | `programs/` |
| 2.11 | **S** Script que submete tx `evaluate_trigger` via `@solana/web3.js` | `programs/` + `api/` |

**Checkpoint Fase 2:** trigger + payout on-chain testados; API expõe status do escrow.
